...
local function setLogicSwitches()
    if settingEnabled(ztsSettings.esc, "arm") then
        ...
        setSetting({"logicalSw", 1}, {func="FUNC_AND", def='"' .. buttonString[ztsSettings.esc.armSwitch+1] .. ',TELEMETRY_STREAMING"', andsw='"NONE"', delay="0", duration="0"})
...
local function setSpecialFunctions()
    ...
    if settingEnabled(ztsSettings.steering, "limit") then
        setSetting({"customFn", 0}, {swtch='"ON"', func="ADJUST_GVAR", def='"0,Src,' .. trimSwitchString[ztsSettings.steering.limitSwitch+1] .. ',1"'})
    end
    if settingEnabled(ztsSettings.steering, "DR") then
        setSetting({"customFn", 1}, {swtch='"ON"', func="ADJUST_GVAR", def='"1,Src,' .. trimSwitchString[ztsSettings.steering.drSwitch+1] .. ',1"'})
    end

    -- brake
    if settingEnabled(ztsSettings.brake, "limit") then
        setSetting({"customFn", 2}, {swtch='"ON"', func="ADJUST_GVAR", def='"2,Src,' .. trimSwitchString[ztsSettings.brake.limitSwitch+1] .. ',1"'})
    end
    if settingEnabled(ztsSettings.brake, "balance") then
        setSetting({"customFn", 3}, {swtch='"ON"', func="ADJUST_GVAR", def='"3,Src,' .. trimSwitchString[ztsSettings.brake.balanceSwitch+1] .. ',1"'})
    end
...
local function setMixers()
    setSetting({"mixData", 0}, {weight="100", destCh=ztsSettings.steering.output, srcRaw="I0", carryTrim="1", mixWarn="0", mltpx="ADD", speedPrec="0", offset="0", swtch='"NONE"', flightModes="000000000", delayUp="0", delayDown="0", speedUp="0", speedDown="0", name= '""'})
    setSetting({"mixData", 1}, {weight="100", destCh=ztsSettings.esc.output, srcRaw="I1", carryTrim="1", mixWarn="0", mltpx="ADD", speedPrec="0", offset="0", swtch='"L4"', flightModes="000000000", delayUp="0", delayDown="0", speedUp="0", speedDown="0", name= '""', curve={type="3", value="1"}})
    setSetting({"mixData", 2}, {weight="100", destCh=ztsSettings.esc.output, srcRaw="I2", carryTrim="1", mixWarn="0", mltpx="ADD", speedPrec="0", offset="0", swtch='"NONE"', flightModes="000000000", delayUp="0", delayDown="0", speedUp="0", speedDown="0", name= '""', curve={type="3", value="2"}})

    if settingEnabled(ztsSettings.brake, "servo") then
        setSetting({"mixData", 3}, {weight="100", destCh=ztsSettings.brake.servoOutput, srcRaw="I3", carryTrim="1", mixWarn="0", mltpx="ADD", speedPrec="0", offset="0", swtch='"NONE"', flightModes="000000000", delayUp="0", delayDown="0", speedUp="0", speedDown="0", name= '""', curve={type="3", value="3"}})
    end
end

local function setOutputs()
    setSetting({"limitData", ztsSettings.steering.output}, {min="0", max="0", ppmCenter="0", offset="0", symetrical="0", revert="0", curve="0", name='"STR"'})
    setSetting({"limitData", ztsSettings.esc.output}, {min="0", max="0", ppmCenter="0", offset="0", symetrical="0", revert="0", curve="0", name='"ESC"'})

    if settingEnabled(ztsSettings.brake, "servo") then
        setSetting({"limitData", ztsSettings.brake.servoOutput}, {min="0", max="0", ppmCenter="0", offset="0", symetrical="0", revert="0", curve="3", name='"FBK"'})
    end
end
...
local function checkGlobalVariable()
    if settingEnabled(ztsSettings.steering, "limit") then setGlobalVariable(modelSettings, 0, "STW", 1024, 924, 0, 1) end
    if settingEnabled(ztsSettings.steering, "DR") then setGlobalVariable(modelSettings, 1, "STE", 0, 100, 0, 1) end
    if settingEnabled(ztsSettings.brake, "limit") then setGlobalVariable(modelSettings, 2, "BRT", 0, 100, 0, 1) end
    if settingEnabled(ztsSettings.brake, "balance") then setGlobalVariable(modelSettings, 3, "BBL", -100, 100, 0, 1) end
end

local function setTelemetryScreen()
    if settingEnabled(ztsSettings.model, "ZTS") and not entryAvailable({"screens", 0, "u", "script", "file"}, modelSettings) then
...
local function setSwitchWarning()
    if settingEnabled(ztsSettings.model, "sWarning") and entryAvailable({"switchWarning"}, modelSettings) then
